:- module(swi_apeal,[widget/2,assertin/1,gui_expansion/2,widget_decl/3, term_expansion/4,op(800, xfx, (widget)),
   op(1100, xfy, '<->'),
   op(1100, xfy, '<='),   
   op(600, xfy, (form)),
   op(600, fy, (unmanaged)),
...
head_expansion((Type widget (Head <-> Decl)), (widget_decl(Type,Head,Decl))):- nonvar(Head),!.
head_expansion(((Type widget Head) <-> Decl), (widget_decl(Type,Head,Decl))):- nonvar(Head),!.
head_expansion('<->'(Head,Decl):- Type, Head:- widget_decl(Type,Head,Decl)):- nonvar(Head),!.
head_expansion('widget'(Type,Head):- Decl, widget_decl(Type,Head,Decl)):- nonvar(Head),!.
head_expansion('<->'(Head,Type), Head:- widget_decl(Type,Head,'<->')):- nonvar(Head),!.
head_expansion('widget'(Type,Head), widget_decl(Type,Head,widget)):- nonvar(Head),!.

gui_expansion(NC,_):- \+ compound(NC),!,fail.
gui_expansion(HeadIn,(:- assertin(Head))):- head_expansion(HeadIn,Head),!.
gui_expansion((HeadIn:-BodyIn),(Head:-BodyIn)):- !, compound(HeadIn), head_expansion(HeadIn,Head).

called_swi_apeal(P):- throw(called_swi_apeal(P)).

assertin(G):- copy_term(G,C),clause(G,_),G=@=C.
assertin(G):- asserta(G).
...
term_expansion(HeadIn,IS,Head,OS):- gui_expansion(HeadIn,Head)->IS=OS.

:- module_transparent(( widget/2,assertin/1,gui_expansion/2,widget_decl/3, term_expansion/4)).
